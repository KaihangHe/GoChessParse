...
class ChessBoardParse:
    __net = SSDNet('models/frozen_inference_graph.pb', 'models/go.pbtxt');

    def __init__(self, frozen_graph_path, pbtxt_path):
        net = SSDNet(frozen_graph_path, pbtxt_path)
    ...
    def __detect(self, srcImage):
        center_list = self.__net.chess_piece_mark(srcImage)
        return center_list

    def __rebuildChessboard(self, srcImage, center_list, padding_val=8):
    ...
    def __validate_edge_line(self, chess_board_pos, edgeImage, center_list):
    ...
    def __houghEdge(self, edgeImage, srcImage=None):
    ...
    def __remapImage(self, edgeImage, corner_list, maskImage, output_Imageshape=(600, 600)):
        #####
        prespect_mat, dstImage = remapImage(edgeImage, corner_list, output_Imageshape)
    ...
    def __shadowHist(self, edgeImage):
        chess_board_pos = []
    ...
    def output_matrix(self, srcImage):
        ...
        edgeImage = self.__rebuildChessboard(srcImage, center_list)
        edge_lines = self.__houghEdge(edgeImage, srcImage)
        corners_list = []
        for index in range(len(edge_lines)):
            corners_list.append(self.__clac_intersection(edge_lines[index], edge_lines[(index + 1) % 4]))
        dstImage, center_list = self.__remapImage(edgeImage, corners_list, maskImage)
        chess_board_pos = self.__shadowHist(dstImage)
        chess_board_pos = self.__validate_edge_line(chess_board_pos, dstImage, center_list)
        output_matrix = self.__position(chess_board_pos, center_list)
        print('output_matrix\n', output_matrix)
...
if __name__ == '__main__':
    ...
    parser.output_matrix('../sample_img/4.png')
    cv2.waitKey()
